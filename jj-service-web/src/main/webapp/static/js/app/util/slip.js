;(function(root, factory) {
    ...
    var slipjs = {
        ...
        _initCommon: function(core,param) { // 作用：初始化
            var that = this;
            that.core = core;
            that.pullTarget = param.pullTarget;
            that.startFun    = param.startFun;
            that.pullFun     = param.pullFun;
            that.pullEndFun     = param.pullEndFun;
        ...
        _pull: function(x, time) {
            var target = this.pullTarget;
            target.style.webkitTransitionDuration = ''+(time || 0)+'ms';
            target.style['webkitTransform'] = 'translate3d(0px, '+x+'px, 0px)';
        },
    ...
    SlipPx._move = function(evt) { // 触摸中
        var that = this,
            ...
            that_x = that.xy;
        if(e.clientX < 5 || e.clientX >document.documentElement.clientWidth-5){
            //终止滑动
            that._end(evt)
            that._movestart = true;
            return;
        }
        if(e.clientY < 5 || e.clientY >document.documentElement.clientHeight-5){
            //终止滑动
            that._end(evt)
            that._movestart = true;
            return;
        ...
        if(!that._movestart){
            ...
            if(that.offset_x >0){
                that.pullFun && that._pull((that._dis_x) / 4);
                that.pullFun && that.pullFun(e);
                return;
            }
            ...
            if(duration < 300 && Math.abs(fast_dist_x) > 10) {
                if (that.xy > -that.up_range || that.xy < that.width_cut_coreWidth) {
                    ...
                    if(that.xy <0){
                        ...
                        that._posTime(that.xy + _momentum.dist >0?0:that.xy + _momentum.dist, _momentum.time);
    ...
    SlipPx._reboundPull = function(time) {
        var that = this;
        that.pullEndFun && that.pullEndFun();
    };
    ...
    SlipPx.toTop = function(callback) {// 返回顶部
        var time = 300;
        this.core.style.webkitTransitionDuration = time + 'ms';
        this._pos(0);
        setTimeout(function() {
            callback.call(SlipPx);
        }, time);
    };
